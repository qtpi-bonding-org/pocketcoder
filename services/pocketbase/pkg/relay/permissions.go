		...
		for scanner.Scan() {
			...
			case "message.updated":
				...
				if info, ok := properties["info"].(map[string]interface{}); ok {
					go func(infoData map[string]interface{}) {
						sessionID, _ := infoData["sessionID"].(string)
						msgID, _ := infoData["id"].(string)
						role, _ := infoData["role"].(string)
						log.Printf("📨 [Relay/SSE] message.updated resolution - sessionID: %s, msgID: %s, role: %s", sessionID, msgID, role)
						chatID := r.resolveChatID(sessionID)
						if chatID != "" {
							// Check for error field in message.updated info
							if errorData, hasError := infoData["error"]; hasError && errorData != nil {
								r.handleMessageError(infoData)
							} else {
								r.handleMessageCompletion(chatID, infoData)
							}
						} else {
			...
			case "message.part.updated":
				...
				if part, ok := properties["part"].(map[string]interface{}); ok {
					go func(partData map[string]interface{}) {
						sessionID, _ := partData["sessionID"].(string)
						partID, _ := partData["id"].(string)
						partType, _ := partData["type"].(string)
						msgID, _ := partData["messageID"].(string)
						r.app.Logger().Info("📦 [Relay/SSE] message.part.updated details", "sessionID", sessionID, "msgID", msgID, "partID", partID, "type", partType)
						chatID := r.resolveChatID(sessionID)
						if chatID != "" {
							r.upsertMessagePart(chatID, partData)
						} else {
			...
			case "message.part.delta":
				// properties = { messageID, sessionID, partID, delta }
				go func(deltaData map[string]interface{}) {
					sessionID, _ := deltaData["sessionID"].(string)
					msgID, _ := deltaData["messageID"].(string)
					partID, _ := deltaData["partID"].(string)
					delta, _ := deltaData["delta"].(string)

					chatID := r.resolveChatID(sessionID)
					if chatID != "" {
						r.applyMessagePartDelta(chatID, msgID, partID, delta)
					}
				}(properties)
...
func (r *RelayService) handleSessionIdle(sessionID string) {
	...
	if turn == "assistant" {
		...
		// 1. Completion Fallback: Mark any "processing" message as completed
		// This prevents UI hangs if OpenCode forgets to send a final message.updated
		processingMsgs, _ := r.app.FindRecordsByFilter("messages", "chat = {:chat} && engine_message_status = 'processing'", "-created", 1, 0, map[string]any{"chat": chatID})
		if len(processingMsgs) > 0 {
			lastMsg := processingMsgs[0]
			ocMsgID := lastMsg.GetString("ai_engine_message_id")
			r.app.Logger().Info("🩹 [Relay/SSE] Cleanup: Marking processing message as completed due to session idle", "ocMsgID", ocMsgID)
			
			// Use handleMessageCompletion to perform the same authoritative sync as a real event
			r.handleMessageCompletion(chatID, map[string]interface{}{
				"id": ocMsgID,
				"role": "assistant",
				"time": map[string]interface{}{"completed": time.Now().Format("2006-01-02 15:04:05.000Z")},
			})
		}
