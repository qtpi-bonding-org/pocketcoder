...
2. Secret delivery with `GOOSE_DISABLE_KEYRING=1` — **RESOLVED (commit 35ea65594).** Keys are read from **env**: the entrypoint sources `$GOOSE_PATH_ROOT/config/keys.env` (rendered by PocketBase) before provider validation. No keyring/secrets file. Path guarded by `config_pipeline.bats`.
3. **(Elevated — gates the schema enum) — RESOLVED (Goose 1.43.0, commit f25795001).** The ACP `session/new` response advertises modes `auto`, `approve`, `smart_approve`, `chat` (current `auto`), captured live. This is an **exact match** for the `poco_configs.mode` enum (§5) — no schema change; the provisional enum was correct.
4. **(New — Opus C1) — RESOLVED (commit 35ea65594).** The pinned goose image runs as `uid=1000(goose) gid=1000(goose)` (confirmed via `id`). PocketBase's `chown` to 1000/1000 (§6.4) targets the correct owner; goose reads the rendered files.
5. **(New)** Whether `config.yaml` (or an env var) supports a **global system prompt / instructions** in the pinned build. If yes, the renderer emits it; if no, the system prompt is inert (documented in §4.1) and the renderer omits it — do **not** invent a config key. `system_prompt`/`Instructions` stays on the record + `SessionProfile` regardless (it becomes live per-chat when recipe-over-ACP lands). **Status:** renderer conservatively omits (no documented global key); left as-is.

> **Live-verification addendum (commit f25795001).** Bringing the stack up against the pinned Goose surfaced three defects that all unit/golden tests passed over, because no PocketBase unit-test helper exists to exercise `FindRecordsByFilter`/ACP serialization: (a) `session/new|load` sent `mcpServers: null` for the default (no-MCP) chat, which Goose rejects with `-32602 "invalid type: null, expected a sequence"` — every prompt failed; (b) `defaultPocoConfigAPI` and (c) `defaultPocoConfig` sorted `poco_configs` by a nonexistent `created` column. The live `tests/agent-c1` acceptance suite is now the regression net for these.
