...
- [x] **Gate A — goose ACP-over-HTTP:** Complete (2026-07-16): Goose v1.36.0, pinned in the spike README, initialized, created a session, streamed a provider-backed prompt, loaded it on a fresh connection, replayed history, completed a second prompt, and accepted `session/cancel` during a streaming turn. Cancellation stopped further chunks and resolved the original prompt with `stopReason: end_turn`. It conforms to the current `Acp-Connection-Id` + connection/session SSE Streamable-HTTP profile. Retain the captured endpoint, authentication, headers, and session ID evidence.
...
- [x] **Gate C — permission pass-through policy set (2026-07-16):** Developer tools work: after `session/set_mode(approve)`, Goose sent `session/request_permission` for a write and read; the spike held each for three seconds, returned the exact offered `allow_always` option, completed the ACP filesystem callbacks, and a fresh `session/load` replayed the completed tool calls and final message. Resolved approvals are represented by completed tool history, not a reissued callback. The gateway half was exercised with a real isolated Docker MCP Gateway (SSE and built-in `mcp-find`): Goose accepted it in `mcpServers` but exposed no gateway tools, even when the gateway had its required Docker socket. This is an attachment/configuration incompatibility, not evidence of an approval bypass. **Policy:** do not enable Cognee or any gateway MCP server by default; reopen this gate only after a pinned Goose configuration/release exposes gateway tools, then repeat the permission test before enabling c3.
...
- [ ] Implement `acp`, `sessions`, `agui`, and API routes with fake ACP tests. Treat the correlated `session/prompt` response as the terminal turn signal after a cancel; test stream bootstrap/replay, live mapping, tool boundary ordering, permission wait/allow/deny/timeout, cancellation, c1 restart, and c2 restart.
