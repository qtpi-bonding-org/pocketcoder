...
- [ ] **Gate A — goose ACP-over-HTTP:** Mostly complete (2026-07-16): Goose v1.36.0, pinned in the spike README, successfully initialized, created a session, streamed a provider-backed prompt, then on a fresh connection loaded the same session, replayed its history, and completed a second prompt. It conforms to the current `Acp-Connection-Id` + connection/session SSE Streamable-HTTP profile. Finish this gate by proving `session/cancel`; retain the captured endpoint, authentication, headers, and session ID evidence.
