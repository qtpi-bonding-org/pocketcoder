...
- [ ] **Gate A — goose ACP-over-HTTP:** With a pinned goose version, use a minimal Go client to initialize, create a session, prompt, receive streaming updates, call `session/load`, and cancel a turn over the actual c1→c2 transport. Record endpoint, authentication, headers, and exact session ID returned.
