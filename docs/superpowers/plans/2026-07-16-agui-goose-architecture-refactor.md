...
- [ ] **Gate C — permission pass-through:** Developer-tool half complete (2026-07-16): after `session/set_mode(approve)`, Goose sent `session/request_permission` for a write and read; the spike held each for three seconds, returned the exact offered `allow_always` option, completed the ACP filesystem callbacks, and a fresh `session/load` replayed the completed tool calls and final message. Resolved approvals are represented by completed tool history, not a reissued permission request. Still verify one gateway MCP operation; if it does not reach this mechanism, explicitly choose gateway enforcement, an allowlist of non-mutating calls, or no Cognee by default.
