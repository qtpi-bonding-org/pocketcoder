...
@test "MCP Infra: PocketBase can restart MCP Gateway container via docker socket" {
    # Validates: MCP_GATEWAY_ARCHITECTURE.md — PocketBase has read-write docker socket
    # and uses it to restart the MCP gateway container.
    #
    # This is a direct infrastructure test — no MCP request/approval involved.
    # We verify PocketBase can reach the docker socket and restart the gateway
    # by hitting the Docker API from inside the PocketBase container.
    ...
    # Verify PocketBase has docker socket mounted
    run docker exec pocketcoder-pocketbase test -S /var/run/docker.sock
    [ "$status" -eq 0 ] || { echo "❌ Docker socket not mounted in PocketBase container" >&2; return 1; }

    # Get the gateway container ID from inside PocketBase (via docker socket)
    local gateway_id
    gateway_id=$(docker exec pocketcoder-pocketbase \
        curl -s --unix-socket /var/run/docker.sock \
        "http://localhost/containers/json?filters=%7B%22name%22%3A%5B%22pocketcoder-mcp-gateway%22%5D%7D" \
        | jq -r '.[0].Id // empty' 2>/dev/null)

    [ -n "$gateway_id" ] && [ "$gateway_id" != "null" ] || {
        echo "❌ PocketBase cannot see MCP gateway via docker socket" >&2
        return 1
    }

    echo "  Gateway container ID: ${gateway_id:0:12}"

    # Restart the gateway from PocketBase via docker socket
    local restart_code
    restart_code=$(docker exec pocketcoder-pocketbase \
        sh -c 'curl -s -o /dev/null -w "%{http_code}" --unix-socket /var/run/docker.sock -X POST "http://localhost/containers/'"$gateway_id"'/restart?t=5"')

    [ "$restart_code" = "204" ] || {
        echo "❌ Docker restart API returned HTTP $restart_code (expected 204)" >&2
        return 1
    }
    ...
    [ "$sse_code" = "200" ] || [ "$sse_code" = "204" ] || {
        echo "❌ Gateway SSE not reachable after restart (HTTP $sse_code)" >&2
        return 1
    }

    echo "✓ PocketBase restarted MCP Gateway via docker socket"
    echo "  Before: $started_before"
    echo "  After:  $started_after"
    echo "  SSE:    HTTP $sse_code (back up)"
}
...
@test "MCP Infra: MCP Gateway spins up MCP server container via Dynamic MCP" {
    ...
    # --- Step 4: Hard-assert a NEW container appeared ---
    #
    # We poll for up to 90 seconds because the gateway may need to pull the image.
    # We look for container names matching "mcp" or the server name.
    #
    ...
    echo "✓ Dynamic MCP spun up a new container for '$server_name' via mcp-add"
}
