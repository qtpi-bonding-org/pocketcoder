...
wait_for_assistant_message() {
    ...
    while [ $(date +%s) -lt $end_time ]; do
        ...
        if [ "${assistant_count:-0}" -gt 0 ]; then
            ...
            # OpenCode may create multiple assistant messages per response, so we need to find the right one
            ...
            if [ "$turn" = "user" ]; then
                ...
                # No message with text found, but turn is user
                # Check if newest message is same as excluded one to avoid infinite loop
                local msg_id
                msg_id=$(echo "$response" | jq -r ".items[0].id" 2>/dev/null)
                
                if [ -n "$exclude_id" ] && [ "$msg_id" = "$exclude_id" ]; then
                    echo "  ℹ Turn is user but newest message ($msg_id) is same as excluded. No new message available." >&2
                    echo ""
                    return 0
                fi
                
                echo "  ℹ Turn is user but no message with text found. Returning newest message $msg_id for caller to handle." >&2
                echo "$msg_id"
                return 0
            fi
