...
func AddHarnessAuthOperations(app core.App, registry *operation.Registry, deps HarnessAuthDeps) {
	...
	registry.Add(operation.Route{OperationID: "getHarnessAuthStatus", Method: http.MethodPost, Path: "/api/pocketcoder/v1/harness-auth/status", Auth: true, Action: func(re *core.RequestEvent) error {
		...
		if input.Harness == "" || input.Provider == "" {
			return pocketCoderError(re, 400, "harness and provider are required")
		}
		...
		if account == nil {
			return re.JSON(200, harnessAuthStatusResp{Harness: input.Harness, Provider: input.Provider, Status: harnessauth.StatusDisconnected, Mode: mode})
		}
		attempt, _ := harnessauth.ActiveAttempt(app, account.Id)
		return re.JSON(200, renderHarnessAuthStatus(account, attempt, mode))
	}})
	...
	registry.Add(operation.Route{OperationID: "startHarnessAuth", Method: http.MethodPost, Path: "/api/pocketcoder/v1/harness-auth/start", Auth: true, Action: func(re *core.RequestEvent) error {
		...
		if _, err := app.FindRecordById("providers", input.Provider); err != nil {
			return pocketCoderError(re, 404, "Provider not found")
		}
		...
		if mode != "oauth" && mode != "none" {
			return pocketCoderError(re, 400, "mode must be one of oauth|none")
		}

		if mode == "none" {
			if err := clearSelectionToNone(app, re.Auth.Id, input.Harness, input.Provider); err != nil {
				log.Printf("[HarnessAuth] clear selection: %v", err)
				return pocketCoderError(re, 500, "Internal error")
			}
			return re.JSON(200, harnessAuthStatusResp{Harness: input.Harness, Provider: input.Provider, Mode: "none", Status: harnessauth.StatusDisconnected})
		}

		// mode == "oauth": spec §4.7's invariant, enforced here (not just by
		// Task 5's schema hook) so the client gets a clear, specific error
		// instead of a generic 500/DB-constraint failure.
		...
		if err != nil || !edge.GetBool("supports_oauth") {
			return pocketCoderError(re, 400, "this provider does not support account login for this harness")
		}
		...
		state, err := runtime.Start(re.Request.Context(), authenticatorKey, stateCtx)
		if err != nil {
	...
	registry.Add(operation.Route{OperationID: "pollHarnessAuth", Method: http.MethodPost, Path: "/api/pocketcoder/v1/harness-auth/poll", Auth: true, Action: func(re *core.RequestEvent) error {
		...
		if input.Provider == "" {
			return pocketCoderError(re, 400, "provider is required")
		}
