...
String computeFingerprintSync(X509Certificate certificate) {
  final cleanBytes = certificate.der;
  final digest = sha256.convert(cleanBytes);
  return digest.bytes
...
void main() {
  ...
  group('CertificateManager - Unit Tests', () {
    group('computeFingerprint', () {
      test('computes SHA-256 fingerprint from certificate DER encoding',
          () async {
        final testCert = _createTestCertificate();
        final fingerprint =
            await certificateManager.computeFingerprint(testCert);
      ...
      test('different certificates produce different fingerprints', () async {
        ...
        final fingerprint1 =
            await certificateManager.computeFingerprint(testCert1);
        final fingerprint2 =
            await certificateManager.computeFingerprint(testCert2);
    ...
    group('validateCertificate', () {
      test('accepts matching fingerprints', () {
        ...
        final result =
            certificateManager.validateCertificate(fingerprint, fingerprint);
      ...
      test('accepts matching fingerprints with different case', () {
        ...
        final result = certificateManager.validateCertificate(
      ...
      test('rejects mismatched fingerprints', () {
        ...
        final result =
            certificateManager.validateCertificate(fingerprint1, fingerprint2);
    ...
    group('storeFingerprint and getFingerprint', () {
      ...
      test('stores fingerprint via secure storage', () async {
        ...
        verify(() => mockSecureStorage.storeCertificateFingerprint(
            instanceId, testFingerprint)).called(1);
      });
      ...
      test('retrieves fingerprint from secure storage', () async {
        ...
        verify(() => mockSecureStorage.getCertificateFingerprint(instanceId))
            .called(1);
      });
    ...
    group('createPinnedClient', () {
      test('creates HttpClient with certificate validation callback', () {
        const expectedFingerprint =
            'aa:bb:cc:dd:ee:ff:00:11:22:33:44:55:66:77:88:99:aa:bb:cc:dd:ee:ff:00:11:22:33:44:55:66:77:88:99';
        final client =
            certificateManager.createPinnedClient(expectedFingerprint);
      ...
      test('pinned client callback accepts matching certificate', () {
        ...
        final testCert = _createTestCertificate();
        computeFingerprintSync(testCert);
    ...
    group('retrieveCertificateFingerprint', () {
      test('throws when connection fails', () async {
        ...
        expect(
          () => certificateManager
              .retrieveCertificateFingerprint('invalid-host-12345.invalid'),
