...
void main() {
  testWidgets('pending DENY is warning-colored, not danger red', (tester) async {
    await tester.pumpWidget(_wrap(_view(const McpServer(
      id: 'pending-1',
      name: 'Pending server',
    ...
    final deny = tester
        .widgetList<TerminalButton>(find.byType(TerminalButton))
        .firstWhere((button) => button.label == 'DENY');
    expect(deny.color, isNot(const Color(0xFFFF3333)));
  });
  ...
  testWidgets('approved REVOKE remains danger red', (tester) async {
    ...
    final revoke = tester
        .widgetList<TerminalButton>(find.byType(TerminalButton))
        .firstWhere((button) => button.label == 'REVOKE');
    expect(revoke.color, const Color(0xFFFF3333));
  });
}
