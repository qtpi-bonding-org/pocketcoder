  ...
  group('LinodeOAuthService', () {
    ...
    group('requiredScopes', () {
      test('returns correct scopes', () {
        expect(service.requiredScopes, equals(['linodes:read_write', 'linodes:create']));
      });
    });
    ...
    group('generateCodeChallenge', () {
      ...
      test('generates different code challenges for different verifiers', () async {
    ...
    group('extractCodeFromCallback', () {
      test('extracts authorization code from callback URL', () {
        final code = service.extractCodeFromCallback('pocketcoder://oauth-callback?code=auth123');
        expect(code, equals('auth123'));
      });

      test('returns null when no code in callback', () {
        final code = service.extractCodeFromCallback('pocketcoder://oauth-callback?error=cancelled');
        expect(code, isNull);
      });

      test('handles URL with multiple parameters', () {
        final code = service.extractCodeFromCallback('pocketcoder://oauth-callback?state=xyz&code=auth123&scope=read');
        expect(code, equals('auth123'));
      });
    });
    ...
    group('exchangeCode', () {
      test('exchanges code for tokens and stores them', () async {
        ...
        verify(() => mockSecureStorage.storeAccessToken(testAccessToken)).called(1);
        verify(() => mockSecureStorage.storeRefreshToken(testRefreshToken)).called(1);
    ...
    group('refreshToken', () {
      ...
      test('throws AuthenticationError when no refresh token available', () async {
        when(() => mockSecureStorage.getRefreshToken()).thenAnswer((_) async => null);
    ...
    group('getAccessToken', () {
      test('returns stored access token when not expired', () async {
        when(() => mockSecureStorage.getAccessToken()).thenAnswer((_) async => testAccessToken);
      ...
      test('refreshes token when within 5 minutes of expiration', () async {
        ...
        verify(() => mockApiClient.refreshAccessToken(testRefreshToken)).called(1);
        verify(() => mockSecureStorage.storeAccessToken(newAccessToken)).called(1);
      });
