...
void main() {
  test('delegates every typed command and extracts the PocketBase host',
      () async {
    ...
    expect(
        runner.calls.map((call) => call.host), everyElement('pb.example.test'));
  ...
  test('wraps runner failures with operation context', () async {
    ...
    await expectLater(
      service.saveBackup(instanceId: 'instance-1'),
      throwsA(isA<ServerControlException>().having(
        (error) => error.message,
        'message',
        contains('saveBackup'),
      )),
