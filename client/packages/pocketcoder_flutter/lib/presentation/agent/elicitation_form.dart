// ElicitationForm (plan Task 13 Step 3): renders the pending
// SessionState.elicitation's requestedSchema as a flat form, and submits
// the field values as an ElicitationResponse.accept via ElicitationCubit.
// Renders nothing when there is no pending elicitation. Nested schema
// objects (oneOf, anyOf, arrays of objects) are out of scope for v1 — the
// schema's top-level "properties" object is treated as a flat list of
// fields, and any non-string/non-number/non-boolean field falls back to a
// string input so the user at least sees what the agent asked for.
...
class ElicitationForm extends StatefulWidget {
  const ElicitationForm({super.key});

  @override
  State<ElicitationForm> createState() => _ElicitationFormState();
}

class _ElicitationFormState extends State<ElicitationForm> {
  ...
  void _submit(BuildContext context, ElicitationResponse resp) {
    // Reset only on success-style responses; the stream-driven
    // elicitation=null from the next watch emission will rerender the
    // form via _resetForNewElicitation when a new one arrives.
