...
///
/// Provides the [AgentConfigCubit] and [ProviderCubit] via nested
/// [BlocProvider]s (the cubit hierarchy follows `AgentManagementScreen`'s
/// pattern, but adds the [ProviderCubit] needed for the harness_model picker
/// since that data lives outside `poco_configs`). The actual view is in
/// [AgentConfigView] so tests can pump just the view with fakes.
class AgentConfigScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      ...
      child: UiFlowListener<AgentConfigCubit, AgentConfigState>(
...
/// The agent-config list + edit dialog UI.
///
/// This widget is intentionally split off from [AgentConfigScreen] so widget
/// tests can wrap it in their own `BlocProvider`s with fake cubits — exactly
/// the same pattern `permission_card_test.dart` uses for `PermissionCard`.
class AgentConfigView extends StatelessWidget {
  const AgentConfigView({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<AgentConfigCubit, AgentConfigState>(
  ...
  String _harnessModelLabelFor(BuildContext context, PocoConfig config) {
    final providerState = context.read<ProviderCubit>().state;
    for (final hm in providerState.harnessModels) {
      if (hm.id == config.harnessModel) {
        return hm.harnessModelId.toUpperCase();
      }
    }
    return config.harnessModel.toUpperCase();
  }
        ...
        return _SelectionField(
          ...
          onTap: () async {
            final picked = await _showListDialog<String>(
              ...
              items: prompts.map((p) => (id: p.id, label: p.name)).toList(),
...
class _HarnessModelPicker extends StatelessWidget {
  const _HarnessModelPicker({
    required this.selectedHarnessModelId,
    required this.onSelected,
  });

  final String? selectedHarnessModelId;
  final ValueChanged<String?> onSelected;
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ProviderCubit, ProviderState>(
      builder: (context, state) {
        final models = state.harnessModels;
        HarnessModel? selected;
        if (selectedHarnessModelId != null) {
          for (final hm in models) {
            if (hm.id == selectedHarnessModelId) {
              selected = hm;
              break;
            }
          }
        ...
        return _SelectionField(
          label: context.l10n.agentConfigHarnessModelLabel,
          currentValue: selected?.harnessModelId.toUpperCase() ??
              context.l10n.agentConfigSelectHarnessModel.toUpperCase(),
          onTap: () async {
            final picked = await _showListDialog<String>(
              context,
              title: context.l10n.agentConfigSelectHarnessModel,
              emptyLabel: context.l10n.agentConfigNoHarnessModels,
              items: models
                  .map((hm) =>
                      (id: hm.id, label: hm.harnessModelId.toUpperCase()))
                  .toList(),
              initialValue: selectedHarnessModelId,
            );
            if (picked != null) onSelected(picked);
          },
        );
